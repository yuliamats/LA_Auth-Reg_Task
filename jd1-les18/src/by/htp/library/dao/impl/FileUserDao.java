...
public class FileUserDao implements UserDao {
	private static final String USERS_PATH = "D:\\Workspace\\JD\\jd1-les18\\src\\resource\\users.txt";
	...
	public boolean registration(User newUser) throws DaoException {
		...
		try {
			users = readUsersFromFile();
			for (User u : users) {
				if ((u.getLogin().compareTo(newUser.getLogin()) == 0) && (u.getPassword().compareTo(newUser.getPassword()) == 0)) {
					return result;
				}
			}
